<template>
    <div style="background-color: rgb(208 211 215);" class="min-h-screen text-slate-800 overflow-hidden relative"
        ...
        <div class="relative z-10 flex flex-col min-h-screen px-5 pt-12 pb-24">
            ...
            <!-- Section: App Icon -->
            <div class="mb-4 rounded-3xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-5 relative overflow-hidden">
                <div class="absolute -left-6 -bottom-6 w-24 h-24 bg-cyan-50 rounded-full opacity-50 pointer-events-none"></div>
                <div class="relative z-10">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <button v-for="icon in iconOptions" :key="icon.value"
                            @click="selectAndSaveIcon(icon.value)"
                            :class="[
                            ...
                            <div :class="['w-10 h-10 rounded-xl flex items-center justify-center', selectedIcon === icon.value ? 'bg-teal-100' : 'bg-white']">
            ...
            <div class="mb-4 rounded-3xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-5">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center justify-between py-3"
                        ...
                        <button @click="isPro ? toggleFailedAttemptsLock() : onProFeatureTap()" :class="[
                            'relative w-12 h-6 rounded-full transition-all duration-300',
                            !isPro ? 'cursor-not-allowed' : '',
                            failedAttemptsLock && isPro ? 'bg-teal-500' : 'bg-slate-200'
                        ]">
                            <span :class="[
                                'absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-all duration-300',
                                failedAttemptsLock && isPro ? 'right-0.5' : 'left-0.5'
                            ]"></span>
                        </button>
                    </div>
                </div>
...
function confirmClearGallery() {
    try {
        ...
        window.onClearGalleryConfirmed = function () { clearGalleryFiles() }
    } catch {
